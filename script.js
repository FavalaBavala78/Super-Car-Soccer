...
const baseCarSpeed = 0.25;
const maxCarSpeed = 1.6;
const boostMultiplier = 1.1; // LOWERED: was 1.15, now makes boost less powerful
const reverseBoostMultiplier = 0.75; // LOWERED: was 0.8
...
function moveCar() {
  ...
  // Handle boost
  if ((keys['Shift'] || keys['shift']) && boost > 0) {
    // Boosting is now less powerful
    targetSpeed = targetSpeed > 0 ? maxCarSpeed * boostMultiplier : -maxCarSpeed * reverseBoostMultiplier;
    boost = Math.max(0, boost - boostDecrement);
    isBoosting = true;
  } else {
    isBoosting = false;
  ...
  // Car air rotation - pitch (forward/backward flip)
  if (isJumping || true) {
    if (keys['w']) car.rotation.x -= 0.05;
    if (keys['s']) car.rotation.x += 0.05;
    car.rotation.x = Math.max(Math.min(car.rotation.x, Math.PI/2), -Math.PI/2);
